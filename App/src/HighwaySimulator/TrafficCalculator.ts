   ...
   private createInitialCars() {
      ...
      for (let i:number = 0; i < lanes.length; i++) {
         ...
         for (let j = 0; j < lanes[i].amountOfCars; j++) {
            var car:Car = new Car(this._p5, new HighwayPosition((Car.LENGTH + lanes[i].distanceBetweeenInitialCars)*j, newLane), this._p5.color("#" + Math.floor((Math.abs(Math.sin((i+1)*(j-2)) * 16777215))).toString(16)), lanes[i].startSpeed)
